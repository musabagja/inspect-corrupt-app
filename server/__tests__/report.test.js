...
describe("GET REPORTS", () => {
  ...
  it("get reports with status 200", (done) => {
    request.get('/')
      .send({
        query: `{reports { _id UserEmail case1 entity province city dateHappened description isDocumentProvided involvedPerson personRole isReported isKeepInTouch aboutInspectApp status }}`
      })
  ...
  it("get reports with all property", (done) => {
    request.get('/')
      ...
      .end((err, res) => {
        ...
        expect(res.body.data.reports[0]).toHaveProperty("case1", expect.any(String));
...
describe("GET REPORT BY ID", () => {
  ...
  it("success get report by id with all property", (done) => {
    request.get('/')
      .send({
        query: `{ report(_id: "${idWantToDelete}") { _id
          UserEmail
          case1
          entity
          province
      ...
      .end((err, res) => {
        ...
        expect(res.body.data.report).toHaveProperty("case1", expect.any(String));
  ...
  it("failed get report by id cause id dont match with any report", (done) => {
    request.get('/')
      .send({
        query: `{ report(_id: "5fd592fc4cgcfe2628ca5f12") { _id
          UserEmail
          case1
          entity
          province
  ...
  it("failed get report by id cause unknown agrument id", (done) => {
    request.get('/')
      .send({
        query: `{ report(id: "5fd592fc4cgcfe2628ca5f12") { _id
          UserEmail
          case1
          entity
          province
...
//         // expect(res.body.data.AddReport).toHaveProperty("aboutInspectApp", expect.any(String));
//         // expect(res.body.data.AddReport).toHaveProperty("status", expect.any(String));
//         done();
//       })
//   })
...
//     request.post('/')
//       .send({
//         query: "mutation {AddReport(payload: { UserId: \"5fd4fa8a2e6d3626001d530a\" case: \"corruption\" entity: \"Indonesian\" province: \"DKI Jakarta\" city: \"Jakarta Pusat\" dateHappened: \"01/01/2020\" description: \"Sri Mulyani Heran, Tunjangan PNS Naik Tapi Masih Ada 'Suap'\" isDocumentProvided: true involvedPerson: [\"Sri mulyani\"] personRole: \"manager\" isReported: true } ) { _id UserEmail case entity province city dateHappened description isDocumentProvided involvedPerson personRole isReported isKeepInTouch aboutInspectApp status }}"
//       })
//       .set("Accept", "application/json")
//       .end((err, res) => {
//         if (err) return done(err)
...
//     request.post('/')
//       .send({
//         query: `mutation {DeleteReport(_id: "${idWantToDelete}") { _id
//           UserEmail
//           case
//           entity
//           province
...
//     request.post('/')
//       .send({
//         query: `mutation {DeleteReport(_id: "5fd5b75753ffd64b788d0dee") { _id
//           UserEmail
//           case
//           entity
//           province
